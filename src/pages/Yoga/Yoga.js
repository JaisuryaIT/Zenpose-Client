...
function Yoga() {
  ...
  const [currentPose, setCurrentPose] = useState('Dog')
  ...
  function startYoga(){
    ...
    function stopPose() {
      ...
      if (bestPerform > 0 || poseTime > 0) {
          const data = {
              pose: currentPose,
              time: (poseTime>0) ? poseTime : bestPerform,
              user_id: token
          };
          ...
          axios.post("https://zenpose.solvixsoftworks.com/api/pose", data)
            ...
            .catch(error => {
                ...
                alert("Error saving pose. Please check your connection.");
            });
      }
      else{
        console.error("Need more time to save he pose");
        alert("More Time Needed to save pose");
      }
  }
